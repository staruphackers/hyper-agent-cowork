...
- **功能與上游一致**：除了介面語言，本平台的行為、資料結構與 API 都跟上游 Paperclip 相同。少數功能擴充：新增 OpenCode 代理人時可選「供應商 API 金鑰」並自動偵測 OpenCode Zen／Go 方案；新增與編輯代理人時可以設定角色、職位與主管（上游只在建立時寫死角色）；`zhtw.7` 起加入一組「切換模型／轉接器」的防護：代理人自己改角色一律要經過同意閘、有執行中的 run 時切換轉接器會先詢問、任務層的模型覆寫在儲存時就檢查是否與 AI 連線相容、Pi／OpenCode／Gemini 的限流與額度用盡會被正確分類為可重試或等待，並提供 `PAPERCLIP_MAX_CONCURRENT_RUNS`（整台主機同時執行上限）與 `PAPERCLIP_LOCAL_ADAPTER_TIMEOUT_SEC`（本機執行預設逾時）兩個環境變數。設計背景見 `doc/plans/2026-09-26-agent-cowork-architecture-review.md`。`zhtw.8` 起一個代理人可以保存多組執行設定（Runtime Profiles：轉接器＋模型＋AI 連線＋環境）並一鍵切換，每組設定各自保留任務對話 session；操作說明見 `doc/zh-TW/VPS-DEPLOY.md` 第 6.2 節，設計見 `doc/plans/2026-09-26-runtime-profiles-phase1.md`。
